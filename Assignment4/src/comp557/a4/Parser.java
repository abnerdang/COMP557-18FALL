...
public class Parser {
	...
	public static Scene createScene(Node dataNode) {
        ...
        for ( int i = 0; i < nodeList.getLength(); i++ ) {
            ...
            } else if ( nodeName.equalsIgnoreCase( "quadrics" ) ) {
            	Quadrics quadrics = Parser.createQuadrics(n);
            	scene.surfaceList.add( quadrics );
            }
        }
        return scene;
	}
	...
	public static Camera createCamera(Node dataNode) {
        ...
        Node focalAttr = dataNode.getAttributes().getNamedItem("focal");
        if ( focalAttr != null ) {
            camera.focalLength = Double.parseDouble( focalAttr.getNodeValue() );        	
        }
	...
	public static Quadrics createQuadrics(Node dataNode) {
		Quadrics quadrics = new Quadrics();
		Node aAttr = dataNode.getAttributes().getNamedItem("a");
		if ( aAttr != null ) {
            quadrics.a = Double.parseDouble(aAttr.getNodeValue());
		}
		Node bAttr = dataNode.getAttributes().getNamedItem("b");
		if ( bAttr != null ) {
			quadrics.b = Double.parseDouble(bAttr.getNodeValue());
		}
		quadrics.material = parseMaterial(dataNode, "material");	
    	return quadrics;
	}
	/**
	 * Create a box object.
