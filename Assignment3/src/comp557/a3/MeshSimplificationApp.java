...
public class MeshSimplificationApp implements SceneGraphNode, Interactor {
    ...
    private String[] soupFiles = {
    		"meshdata/tetrahedron.obj", //0
    		"meshdata/topologytest.obj", //1
    		"meshdata/ico-sphere-tris.obj", //2
    		"meshdata/icosphere2.obj", //3
    		"meshdata/cube.obj", //4
    		"meshdata/cube2obj.obj", //5
    		"meshdata/bunny.obj", //6
    		"meshdata/cow.obj", //7	
            "meshdata/monkey.obj", //8  
        };
    
    public MeshSimplificationApp() {    
        loadSoupBuildAndSubdivide( soupFiles[8] );
        EasyViewer ev = new EasyViewer("Comp 557 Mesh Simplification - Bo Dang", this, new Dimension(1000, 1000), new Dimension(600, 600) );
        ...
        component.addKeyListener(new KeyAdapter() {
            ...
            public void keyPressed(KeyEvent e) {
                if (e.getKeyCode() == KeyEvent.VK_SPACE) {
                	...
                	if ( e.isShiftDown() ) {
                		do {
                			int faceNum = heds.faces.size();
                			heds.meshSimplification(regularizationWeight.getValue());
                			System.out.println("Test shift c");
                			if((faceNum-heds.faces.size()) == 0) break;
                        	// TODO: Objective 2: handle C k	eypress to collapse until it is impossible to make further simplificaitons
                			// if ( noMoreCollapse ) break;
                		} while ( true );
                	} else {
                		heds.meshSimplification(regularizationWeight.getValue());
                		if(!heds.edges.isEmpty()) currentHE = heds.edges.peek().he;
